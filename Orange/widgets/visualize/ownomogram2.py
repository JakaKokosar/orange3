...
class ContinuousItemMixin:
    ...
    def _get_tooltip_label_value(self):
        ...
        if not delta:
            return np.nan
        print('OLA:', self.value, self.value / 52.99550269069998, start + self.value * (stop - start) / delta)
        # print('_get_tooltip_label_value', )
...
class ProbabilitiesRulerItem(QGraphicsWidget):
    ...
    def __init__(self, name, values, scale, name_offset, offset, max_log_partial_hazard, get_points,
        ...
        self.prob_items = [
            (i, QGraphicsTextItem(" " + str(i) + " "),
             QGraphicsLineItem(0, 0, 0, 0)) for i in np.round(np.linspace(0, max_log_partial_hazard, num=15), 2)]

    def rescale(self):
        shown_items = []
        for prob, text, tick in self.prob_items:
            # print(prob, text, tick, self.get_points(prob))
...
class OWNomogram2(OWWidget):
    ...
    def __init__(self):
        ...
        self.align = OWNomogram2.ALIGN_LEFT
    ...
    def update_controls(self):
        ...
        item.setFlags(item.flags() | Qt.ItemIsEnabled)
        # self.align = OWNomogram2.ALIGN_LEFT # OWNomogram2.ALIGN_ZERO
    ...
    def calculate_log_reg_coefficients(self):
        ...
        # [ -x if x > 1 else x for x in self.classifier.coefficients ]
        coeffs = np.array(self.classifier.coefficients).reshape(-1, 1)
        # coeffs = np.vstack((coeffs * (-1), coeffs))
        # self.log_reg_coeffs = [coeffs[:, r] for r in ranges]
        # self.log_reg_coeffs = [coeffs[:, r] for r in ranges]
        # self.log_reg_coeffs = np.array(self.classifier.coefficients).reshape(-1,1)
        # if len(self.domain.class_var.values) == 2:
        #    self.b0 = np.hstack((self.b0 * (-1), self.b0))
        #    coeffs = np.vstack((coeffs * (-1), coeffs))
        # self.log_reg_coeffs = [coeffs[:, r] for r in ranges]
        self.log_reg_coeffs_orig = coeffs.copy()

        min_values = np.zeros(4)
        max_values = nanmax(self.data.X, axis=0)

        for min_val, max_val, c in zip(min_values, max_values, coeffs.flat):
            self.log_reg_cont_data_extremes.append(sorted([min_val, max_val], reverse=(c < 0)))

        self.log_reg_coeffs = np.hstack(
            # (coeffs * min_values.reshape(-1, 1), coeffs * max_values.reshape(-1, 1))
            (min_values.reshape(-1, 1), coeffs * max_values.reshape(-1, 1))
        )
        print()
    ...
    def update_scene(self):
        ...
        probs_text = QGraphicsTextItem("Linear predictor")
        ...
        min_points, max_points = sum(points)
        # KLE JE BIL ALIGN LEFT IF
        ...
        probs_item, nomogram_foot = self.create_footer_nomogram(
            probs_text, d, minimums, max_width, name_offset, min_points, max_points)
    ...
    def _adjust_scale(attributes, points, max_width, diff, attr_inds,
        ...
        names = list(chain.from_iterable(
            [_get_labels(a, lr and lr[i],
                         OWNomogram2.get_ruler_values(p.min(), p.max(),
                                                      scale * p.ptp(), False))
    ...
    def create_main_nomogram(self, attributes, attr_inds, name_items, points,
        ...
        for i, attr, name_item, point in zip(attr_inds, attributes, name_items, points):
            if attr.is_discrete:
                ...
                data_extremes = self.log_reg_cont_data_extremes[i]
                values = self.get_ruler_values(
                ...
                feature_items.append(
                    cont_feature_item_class(
                        name=name_item,
                        variable=attr,
                        data_extremes=data_extremes,
                        values=values,
                        scale=scale_x,
                        name_offset=name_offset,
                        offset=-scale_x * min_p)
                )
    ...
    def get_ordered_attributes(self):
        ...
        if sort_by == SortBy.NO_SORTING:
            ...
            def key(x):
                i, attr = x
                if attr.is_discrete:
                    ptp = self.points[i].ptp()
                else:
                    coef = np.abs(self.log_reg_coeffs_orig[i]).mean()
                    ptp = coef * np.ptp(self.log_reg_cont_data_extremes[i])
                return -ptp
            ...
            def key(x):
                i, attr = x
                max_value = (self.points[i].max()
                             if attr.is_discrete else
                             np.mean(self.log_reg_cont_data_extremes[i]))
                return -max_value
    ...
    def create_footer_nomogram(self, probs_text, d, minimums,
                               max_width, name_offset, min_points, max_points):
        print('d', d)
        values = self.get_ruler_values(min_points, max_points, max_width)
        min_sum, max_sum = min(values), max(values)
        diff_ = np.nan_to_num(max_sum - min_sum)
        scale_x = max_width / diff_ if diff_ else max_width

        max_log_partial_hazard = max_points / d
    ...
    #         if self.align == OWNomogram2.ALIGN_LEFT:
    ...
    #         if self.scale == OWNomogram2.POINT_SCALE:
    #             total *= d
    #         totals[i] = total
    ...
    #     # d_ = d
    #     # #
    #     values = self.get_ruler_values(100, 10, max_width)
    #     # # min_sum, max_sum = min(values), max(values)
    #     diff_ = np.nan_to_num(100 - 10)
    #     scale_x = max_width / diff_ if diff_ else max_width
    ...
    #
    #     probs_item = ProbabilitiesRulerItem(
    #         probs_text, values, scale_x, name_offset, - scale_x * 10,
    #         get_points=get_points,
    #         title='asd',
    #         # title="{}='{}'".format(cls_var.name, cls_var.values[cls_index]),
    #         get_probabilities=get_normalized_probabilities)
    ...
    def _init_feature_marker_values(self):
        ...
        for i, attr in enumerate(self.domain.attributes):
            ...
            if feature_val is not None:
                value = (self.points[i][int(feature_val)]
                         if attr.is_discrete else
                         self.log_reg_coeffs_orig[i][0] * feature_val)
            values.append(value)
...
if __name__ == "__main__":  # pragma: no cover
    ...
    WidgetPreview(OWNomogram2).run(set_classifier=clf, set_data=data)
